...
**Решение:** Программный детектор, проверяющий что все капитализированные сущности (названия компаний, продуктов, персон) присутствуют в исходных источниках.

**Как работает (шаг 6.4 в пайплайне):**
1. Извлекает все proper nouns из исходных источников (OpenAI, Google, TechCrunch, Claude, etc.)
2. Извлекает все proper nouns из сгенерированного черновика
3. Находит сущности в черновике, отсутствующие в источниках
4. Использует Levenshtein distance для поиска похожих ошибок (Devika ≈ Devin)

**Результаты:**
- ❌ **Критические** (совсем неизвестные): TechStartup, FakeCompany → **блокирует статью**
- ⚠️  **Предупреждения** (похожие): Devika когда есть Devin → логирует, но продолжает
- ✅ **Подтверждены**: все сущности в источниках → пропускает

**Интеграция в пайплайн:**
- Запускается после content-writer (шаг 6.4)
- Перед fact-checker (шаг 6.5)
- При обнаружении критических ошибок → sys.exit(1) (останавливает генерацию)
